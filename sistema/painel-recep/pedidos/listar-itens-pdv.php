...
$query_con = $pdo->query("SELECT * FROM pedidos WHERE empresa = '$id_empresa' and id = '$id_pedido' order by id desc");
...
$total_venda = 0;
$total_vendaF = 0;
$query_con = $pdo->query("SELECT * FROM itens_pedidos WHERE empresa = '$id_empresa' and pedido = '$id_pedido' order by id desc");
$res = $query_con->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
if($total_reg > 0){ 
...
if($tipo  == 'Produto'){
$query2 = $pdo->query("SELECT * FROM produtos where empresa = '$id_empresa' and id = '$id_item'");
...
$pasta = 'produtos';
	...
	$query2 = $pdo->query("SELECT * FROM pratos where empresa = '$id_empresa' and id = '$id_item'");
$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
