...
<small>
	<table id="example" class="table table-hover table-sm my-4" style="width:98%;">
		...
		<tbody>
			<?php 
			$query = $pdo->query("SELECT * FROM imagens where empresa = '$id_empresa' order by id desc");
			...
			for($i=0; $i < @count($res); $i++){
				foreach ($res[$i] as $key => $value){	}
					...
					$query2 = $pdo->query("SELECT * FROM categorias where empresa = '$id_empresa' and id = '$categoria'");
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							if(@$_GET['funcao'] == 'novo'){
								...
								$id = @$_GET['id'];
								$query = $pdo->query("SELECT * FROM imagens WHERE empresa = '$id_empresa' and id = '$id'");
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
