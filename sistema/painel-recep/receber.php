...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from contas_receber where empresa = '$id_empresa' order by id desc");
			...
			<table id="example" class="table table-hover my-4" style="width:100%">
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						$query_p = $pdo->query("SELECT * from usuarios where empresa = '$id_empresa' and id = '$id_usu'");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from contas_receber where empresa = '$id_empresa' and id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
