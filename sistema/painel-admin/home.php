...
$query = $pdo->query("SELECT * from produtos where empresa = '$id_empresa'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_produtos = @count($res);



$query = $pdo->query("SELECT * from fornecedores where empresa = '$id_empresa'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_fornecedores = @count($res);



$query = @$pdo->query("SELECT * from produtos where empresa = '$id_empresa' and estoque < '$nivel_estoque'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
...
$saldo = 0;
$saldoF = 0;
$query = $pdo->query("SELECT * from movimentacoes where empresa = '$id_empresa' and data = curDate() order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
if($total_reg > 0){ 
	...
	$query = $pdo->query("SELECT * from movimentacoes where empresa = '$id_empresa' order by id desc limit 1");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	...
	$query = $pdo->query("SELECT * from contas_receber where empresa = '$id_empresa' and vencimento < curDate() and pago != 'Sim'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	...
	$query = $pdo->query("SELECT * from contas_pagar where empresa = '$id_empresa' and vencimento < curDate() and pago != 'Sim'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	...
	$saidasM = 0;
	$saldoM = 0;
	$query = $pdo->query("SELECT * from movimentacoes where empresa = '$id_empresa' and data >= '$dataInicioMes' and data <= curDate() ");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		...
		$totalPagarM = 0;
		$query = $pdo->query("SELECT * from contas_pagar where empresa = '$id_empresa' and data >= '$dataInicioMes' and data <= curDate()");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		$pagarMes = @count($res);
		$total_reg = @count($res);
		if($total_reg > 0){ 
		...
		$totalReceberM = 0;
		$query = $pdo->query("SELECT * from contas_receber where empresa = '$id_empresa' and data >= '$dataInicioMes' and data <= curDate()");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		...
		$totalVendasM = 0;
		$query = $pdo->query("SELECT * from reservas where empresa = '$id_empresa' and data >= '$dataInicioMes' and data <= '$dataMesFinal'");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		...
		$total_pedidos = 0;
		$query = $pdo->query("SELECT * from pedidos where empresa = '$id_empresa' and data >= '$dataInicioMes' and data <= '$dataMesFinal'");
		$res = $query->fetchAll(PDO::FETCH_ASSOC);
		$total_ped = @count($res);
		if($total_ped > 0){ 
