	...
	<table id="example" class="table table-hover table-sm my-4" style="width:98%;">
		...
		<tbody>
			...
			for($i=0; $i < @count($res); $i++){
				...
				//BUSCAR O NOME RELACIONADO
				$query2 = $pdo->query("SELECT * FROM fornecedores where empresa = '$id_empresa' and id = '$id_forn'");
				$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
				if(@count($res2) == 0){
				...
				<tr>
					...
					<td><img src="../img/<?php echo $pagina ?>/<?php echo @$res[$i]['imagem']?> "></td>
					<td>
						<a href="index.php?pag=<?php echo $pagina ?>&funcao=editar&id=<?php echo $id_reg ?>" title="Editar Registro">
							...
							<a href="index.php?pag=<?php echo $pagina ?>&funcao=excluir&id=<?php echo $id_reg ?>" title="Excluir Registro">
								...
								<a href="" onclick="dados('<?php echo @$res[$i]["nome"] ?>', '<?php echo @$valor_compra ?>', '<?php echo $valor_venda ?>', '<?php echo $nome_cat ?>', '<?php echo $nome_forn ?>', '<?php echo $res[$i]["estoque"] ?>', '<?php echo @$res[$i]["imagem"] ?>')" title="Ver Dados">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							if(@$_GET['funcao'] == 'novo'){
								...
								$id = @$_GET['id'];
								$query = $pdo->query("SELECT * FROM produtos WHERE empresa = '$id_empresa' id = '$id'");
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
								$nome = @$res[0]['nome'];
